...
import { X, User as UserIcon, Phone, History, Calendar, Clock, CheckCircle2, Clock3 } from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-md bg-white/5 border border-gold/20 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-xl"
        >
          {/* Header */}
                ...
                {mockHistory.map((item, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="text-gold font-bold text-sm uppercase tracking-wider">{t[item.service]}</p>
                      ...
                      <div className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter ${
                        ...
                        {item.status === 'completed' ? t.profile.completed : t.pending}
                      </div>
                    </div>
